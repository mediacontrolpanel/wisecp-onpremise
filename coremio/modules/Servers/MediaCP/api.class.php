<?php
    class MediaCPAPI
    {
        ...
        public function call($path = '', $method = NULL, $data = [], bool $catchHttpCodeErrors = true)
        {
            ...
            if ( !$method && is_array($data) && count($data) > 0) $url .= '?'.http_build_query($data);
            curl_setopt($ch, CURLOPT_URL, $url);

            $responseData = curl_exec($ch);
            $this->response = $responseData;
            Modules::save_log("Servers","MediaCP",$url,$data,$responseData);
            ...
            if ($responseData === false || (curl_getinfo($ch, CURLINFO_HTTP_CODE) !== 200 && $catchHttpCodeErrors)) {
                ...
                Modules::save_log("Servers","MediaCP",$url,$data,$this->error);
                return false;
            }
            curl_close($ch);
